...
#[test]
fn add_function() {
  let mut some_t = Task::new(Heap::default(), "global.add", vec![Value::U64(4), Value::U64(8)], None);
...
#[test]
fn sub_add_function() {
  let mut some_t = Task::new(Heap::default(), "global.subAdd", vec![Value::U64(6), Value::U64(5), Value::U64(4)], None);
  let result = some_t.run();
...
fn factorial_function() {
  let mut some_t = Task::new(Heap::default(), "global.factorial", vec![Value::U64(3)], None);
  let result = some_t.run();
...
fn b_search_function() {
  ...
  let mut some_t =
    Task::new(heap, "global.binary_search", vec![Value::U64(4), Value::U64(0), Value::U64(elements_len - 1)], None);
  let result = some_t.run();
