...
#[test]
fn add_function() {
  let mut some_t = Fiber::new();
  some_t.load_task("global.add", vec![Value::U64(4), Value::U64(8)], None);
...
fn sub_add_function() {
  let mut some_t = Fiber::new();
  some_t.load_task("global.subAdd", vec![Value::U64(6), Value::U64(5), Value::U64(4)], None);
  let result = some_t.run();
...
fn factorial_function() {
  let mut some_t = Fiber::new();
  some_t.load_task("global.factorial", vec![Value::U64(3)], None);
  let result = some_t.run();
...
fn b_search_function() {
  ...
  let mut some_t = Fiber::new_with_heap(heap);

  some_t.load_task("global.binary_search", vec![Value::U64(4), Value::U64(0), Value::U64(elements_len - 1)], None);
  let result = some_t.run();
  assert_eq!(RunResult::Done(Value::OptionU64(Some(3))), result);

  some_t.load_task("global.binary_search", vec![Value::U64(10), Value::U64(0), Value::U64(elements_len - 1)], None);
  let result = some_t.run();
